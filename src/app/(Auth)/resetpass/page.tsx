...
export default function ResetPassword() {
    ...
    async function onSubmit(values: { password: string; confirm: string }) {
        ...
        try {
            ...
            const res = await axios.put("https://ecommerce.routemisr.com/api/v1/auth/resetPassword", payload);
            ...
            <div className="w-full max-w-md p-8 bg-white rounded-lg shadow">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <div>
                        ...
                        <input type="password" {...register("password", { required: true })} className="w-full mt-1 border rounded px-3 py-2" />
                    </div>
                    <div>
                        <label className="block text-sm">Confirm Password</label>
                        <input type="password" {...register("confirm", { required: true })} className="w-full mt-1 border rounded px-3 py-2" />
                    </div>
